...
#hadoop-aws package needed. Correct version picked
os.environ['PYSPARK_SUBMIT_ARGS'] = '--packages org.apache.hadoop:hadoop-aws:3.3.2 pyspark-shell'

#creating spark session for later spark use
spark = SparkSession.builder.master("local[*]").getOrCreate()

#s3 secret and access keys are inputted so we receive data from it.
...
#csv files pulled from s3 storage
cum_rus_eq_losses = load_s3_data("s3a://bucket/russia_losses_equipment.csv", spark)
cum_rus_pers_losses = load_s3_data("s3a://bucket/russia_losses_personnel.csv", spark)

#create a more refined dataframe from df pulled from s3 (selected columns)
...
#calls function that makes it so that the values aren't cumulative anymore
...
#create a view of the current dataframes after cleaning.
...
#database functions called to input data into MYSQL
